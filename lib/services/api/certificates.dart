...
class CertificateService extends GetxController {
  ...
  Future<ProjectSubmission> submitProject({
    ...
    try {
      ...
      if (message.contains('Paid subscription is required')) {
        throw ApiException(
          'Paid subscription is required to submit a project for this course.',
        );
      }
  ...
  String _extractErrorMessage(dynamic data) {
    if (data is Map) {
      ...
      if (data['device'] is List && (data['device'] as List).isNotEmpty) {
        return (data['device'] as List).first.toString();
      }
      if (data['detail'] != null) {
